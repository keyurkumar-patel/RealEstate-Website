...
public partial class Admin_ReceiptForm : System.Web.UI.Page
{
    SqlConnection cn;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            else
            {
                ...
                SqlCommand cmdcity = new SqlCommand("SELECT City.CityId FROM AdminLogin INNER JOIN  City ON AdminLogin.City = City.CityName WHERE (AdminLogin.AdminId ='" + Session["AdminLogin"].ToString() + "')", cn);
                cn.Open();
    ...
    protected void DDList_CustomerId_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            SqlCommand cmdrece = new SqlCommand("Select Name from AdminLogin where AdminId='" + Session["AdminLogin"].ToString() + "'", cn);

            SqlDataReader drrece = cmdrece.ExecuteReader();
            if (drrece.HasRows)
            {
                drrece.Read();
                lb_ReceivedBy.Text = drrece["Name"].ToString();
                drrece.Close();
                cn.Close();
            }
